...
const StepThree: FC = () => {
    ...
    useEffect(() => {
        if(formikContext.values.numberNotifications > 0){
            setDataNumber(formikContext.values.numberNotifications)
            formikContext.values.notificationsContacts.map((element: string, index: number) => {
                if(index % 2 === 0){
                    formikContext.setFieldValue(`notificationEmail${index%2}`, element)
                }
                else formikContext.setFieldValue(`notificationPhoneMain${(index-1)/2}`, element)
            })
        }
    }, [])
